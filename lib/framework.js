...
const drawBow = (semiCircle, height) => {
  let center = [0, 0, 0];
  let vertices = [];
  let indexes = [];
  let iterator = 0;
  for (iterator = 3*Math.PI/2; iterator <= 5*Math.PI/2; iterator += 0.001) {
    vertices.push(semiCircle * Math.cos(iterator) + center[0]);
    vertices.push(height * Math.sin(iterator) + center[1]);
    vertices.push(center[2]);
  }
  for (iterator = 1; iterator < vertices.length; iterator++) {
    indexes.push(iterator-1);
    indexes.push(iterator);
  }
...
class Mesh2dBow {
  constructor (name, colors, semiCircle, height) {
    const res = drawBow(semiCircle, height);
    this.vertexPositions = res.vertices;
    this.vertexColors = colors;
    this.indices = res.indexes;
